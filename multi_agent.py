...
def math(state: State): 
    ...
    model, math_assistant_tokenizer = FastLanguageModel.from_pretrained(
        model_name = "./math_assistant", # YOUR MODEL YOU USED FOR TRAINING
        max_seq_length = 2000,
    ...
    prompt = math_assistant_tokenizer.apply_chat_template(message, tokenize = True, add_generation_prompt = True, return_tensors = "pt").to("cuda")
...
#     math_assistant_model_id = "./math_assistant"    
#     math_assistant_tokenizer = AutoTokenizer.from_pretrained(math_assistant_model_id, truncation=True, model_max_length=512)    

#     # Create pipeline
#     # model_kwargs={"quantization_config": BitsAndBytesConfig(load_in_8bit=True, llm_int8_enable_fp32_cpu_offload=True)}
#     math_assistant_pipeline = pipeline(
#         "text-generation",
#         model=math_assistant_model_id,
#         tokenizer=math_assistant_tokenizer,
#         device_map="auto",        
#         batch_size=32,
...
#     prompt = math_assistant_tokenizer.apply_chat_template(message, add_generation_prompt=True, tokenize=False)
    
#     # Generate text
#     sequences = math_assistant_pipeline(
#         prompt,
#         do_sample=True,
#         temperature=0.7,
#         top_p=0.9,
...
#     )
#     answer = sequences[0]["generated_text"].split("<|im_sep|>")[3]
#     answer = answer.replace('\n', '')
...
def english(state: State):   
    ...
    english_assistant_model_id = "Qwen/Qwen2-7B-Instruct"
    english_assistant_tokenizer = AutoTokenizer.from_pretrained(english_assistant_model_id, truncation=True, model_max_length=512)    

    # Create pipeline
    english_assistant_pipeline = pipeline(
        "text-generation",
        model=english_assistant_model_id,
        tokenizer=english_assistant_tokenizer,
        device_map="auto",
        torch_dtype=torch.bfloat16,
        model_kwargs={"quantization_config": BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=torch.bfloat16)}
    )    
...
def retrieve(state: State):
    torch.cuda.empty_cache()
    gc.collect()
    retrieved_docs = make_rag().vectorstore.similarity_search(state["messages"][0].content,  k=2)
    gc.collect()
...
def korea_history(state: State):  
    ...
    korea_history_model_id = "Qwen/Qwen2-7B-Instruct"
    korea_history_tokenizer = AutoTokenizer.from_pretrained(korea_history_model_id, truncation=True, model_max_length=1024)
    korea_history_pipeline = pipeline(
        "text-generation",
        model=korea_history_model_id,
        tokenizer=korea_history_tokenizer,
        device_map="auto",
        torch_dtype=torch.bfloat16,
        model_kwargs={"quantization_config": BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=torch.bfloat16)}
    )
    
    prompt = korea_history_tokenizer.apply_chat_template(message, add_generation_prompt=True, tokenize=False)
...
def find_target(state: State):
    ...
    find_target_model_id = "Qwen/Qwen2-7B-Instruct"
    find_target_tokenizer = AutoTokenizer.from_pretrained(find_target_model_id)

    find_target_pipeline = pipeline(
        "text-generation",
        model=find_target_model_id,
        tokenizer=find_target_tokenizer,
        device_map="auto",
        batch_size=32,
        torch_dtype=torch.bfloat16,
        model_kwargs={"quantization_config": BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=torch.bfloat16)}
    ...
    prompt = find_target_tokenizer.apply_chat_template(message, add_generation_prompt=True, tokenize=False)
